...
Ingestion працює — python ingest.py будує FAISS-індекс (так само як у hw5)
Planner декомпозує — запит користувача розбивається у структурований ResearchPlan
Researcher виконує — слідує плану, використовує web + knowledge base
Critic оцінює — повертає структурований CritiqueResult з verdict
Ітерація працює — якщо Critic каже REVISE, Researcher повертається з конкретним зворотним зв'язком
HITL працює — коли Supervisor викликає save_report, користувач бачить звіт і затверджує/відхиляє
Звіт збережено — після затвердження звіт зберігається у ./output/
